...
When two language models collaborate, *e.g.,* two agents communicating, they exchange information in natural language, one token at a time. This is a natural interface, but a lossy one. To emit a token, a model reduces its full distribution over the vocabulary into a single choice, discarding the uncertainty and structure that its intermediate activations encode. A model's richest representation of a problem lives in those activations. The final decoding step discards most of it, because next-token prediction does not require it. However, that discarded information is can be valuable.
